...
void FLuaContext::RegisterDelegates()
{
	...
	FCoreDelegates::OnPostEngineInit.AddRaw(
		GLuaCxt, &FLuaContext::OnPostEngineInit);	// called before FCoreDelegates::OnFEngineLoopInitComplete.Broadcast(), after
													 // GEngine->Init(...)
...
#endif
			FString LibPath = FString::Printf(TEXT("%s/Source/ThirdParty/Lua/binaries/%s/%s"),
				*IPluginManager::Get().FindPlugin(TEXT("UnLua"))->GetBaseDir(), *PlatformName, *LibName);
...
bool FLuaContext::ExportClass(UnLua::IExportedClass* Class)
{
	if (Class)
	{
		TMap<FName, UnLua::IExportedClass*>& ExportedClasses =
			Class->IsReflected() ? ExportedReflectedClasses : ExportedNonReflectedClasses;
...
void FLuaContext::OnPostEngineInit()
{
	...
	if (!GIsEditor)
	{
		...
		{
			GameViewportClient->OnGameViewportInputKey().BindRaw(
				this, &FLuaContext::OnGameViewportInputKey);	// bind a default input event
		}
	}
#endif
}
...
void FLuaContext::PostLoadMapWithWorld(UWorld* World)
{
	...
	OnActorSpawnedHandle =
		World->AddOnActorSpawnedHandler(FOnActorSpawned::FDelegate::CreateUObject(Manager, &UUnLuaManager::OnActorSpawned));
...
void FLuaContext::Cleanup(bool bFullCleanup, UWorld* World)
{
	...
	if (L)
	{
		...
		else
		{
			...
			OnPostGarbageCollectHandle =
				FCoreUObjectDelegates::GetPostGarbageCollect().AddRaw(this, &FLuaContext::OnPostGarbageCollect);
